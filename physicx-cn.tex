...
\usepackage[physics,fixdif]{physicx}
...
\newcommand{\issue}[1]{(\href{https://github.com/Sophanatprime/physicx/issues/#1}{\texttt{\##1}})}

\author{雾月, Longaster\thanks{Email: longaster@163.com}}
\title{\pkg{physicx}，扩展的 \pkg{physics} 宏包}
\date{\today\quad v0.3.6}

\changes{v0.3.4}{2022/09/13}{修复文档错误。}
...
\changes{v0.3.3}{2022/07/30}{对 \pkg{fixdif} 宏包提供支持。}
\changes{v0.3.6}{2023/02/02}{适配 \pkg{fixdif} Version 2.0a。}
\opt{fixdif} 用于控制是否接入 \pkg{fixdif} 宏包的功能，当这个选项设置为真时，用户必须自行加载 \pkg{fixdif} 宏包，若还启用了 \opt{physics} 选项，则将微分号和偏微分号替换为 \pkg{fixdif} 的实现。

\changes{v0.3.4}{2022/09/13}{新增 \opt{original} 选项。\issue{8}}
\opt{original} 用于恢复被 \pkg{physics} 宏包重定义的 \tn{div}、\tn{Re}、\tn{Im} 命令。仅当使用了 \opt{physics} 选项或加载了 \pkg{physics} 宏包时生效。
...
\changes{v0.3.5}{2022/10/22}{size现在可以接受数值值。}
\begin{function}{
    ...
    \opt{left-size} 和 \opt{right-size} 的参数可以为诸如 \tn{left}、\tn{Bigl}、
    \tn{right}、\tn{Bigr} 等值，也可以是一个数值，此时，它表示这个括号的高度为这个数值乘以
一个标准括号高度。
...
\changes{v0.3.6}{2022/11/16}{quantity新增auto键。}
\begin{function}{
    size,auto,noauto,args,args*,
}
    \opt{size} 同时设置 \opt{left-size} 和 \opt{right-size}。
    
    \opt{auto} 设置 \verb+left-size=\left, right-size=\right+。

    \opt{noauto} 将 \opt{size} 设置为空。一般情况下，可以不写 \verb|size=|，而只写它的值。
...
\begin{example}[]
\makeatletter
%% 定义了一个 \spxqty，它的第一个参数为可选的键值参数，
%% 第二个用于控制字体大小，当不给出或者为0时，使用text字体，当为1时，使用script字体，
%% 当为2时，使用scriptscript字体，否则为display字体
%% 其后的值为内容，除了可以使用 {} 外，也支持 \bgroup \egroup。这在其它命令中是不支持的
\newbox\spxqty@box \newcount\spxqty@int
\newcommand{\spxqty}[1][]{%
  \def\spxqty@opt{#1}%
...
\begin{function}{\diagonalmatrix}
    ...
    \cs{physicxtmp} 宏中，将其展开一次即可得到矩阵的值（不包括外部的 
    \verb|\begin{matrix}| 等）。
...
\begin{function}{expand}
    ...
    \end{syntax}
    对输入的元素进行展开，即先对 \opt{array}、\opt{diag}、\opt{item} 展开，再进行处理。
\end{function}
...
\begin{function}[updated=2022-04-27]{element-code, element-code*}
    \begin{syntax}
        ...
        ~~~~~~~~~~~~~~~~~except-regex|only-regex|\meta{macro name}>
    \end{syntax}
    \opt{element-code} 使用三个参数，分别为 \verb|main|、\verb|行角标|、\verb|列角标|。此
    选项设置每一个矩阵元素要显示的代码。\cs{physicx@matrixelement} 宏保存这 \meta{code}（带有三个参数）。

    \opt{element-code*} 控制要排除的元素，这些元素不使用 \opt{element-code} 中的处理代码。
    一般情况下，应该首先使用 \opt{element-code}，再使用 \opt{element-code*}，如果先使用 
    \opt{element-code*}，再使用 \opt{element-code}，则 \opt{element-code*} 不会生效。
  \begin{itemize}
    \item \opt{except-empty} 将排除空元素（完全为空，不包含任何字符）；
    \item \opt{except-blank} 排除空白元素（包括空格）；
    \item \opt{element-dots} 排除 \tn{cdots}、\tn{vdots}、\tn{ldots}、\tn{ddots}；
    \item \opt{except-tl} 排除 \cs{physicxexcept} 的中记号，如果字符是一个整体要被排除，应该使用花括号：\verb|\def\physicxexcept{{abc}{ABC}}|，将排除元素为 \verb|abc| 或 \verb|ABC| 的情况；
    \item \opt{except-regex} 将排除那些匹配正则表达式的元素，这个正则表达式保存在 \cs{physicxexcept} 中；
    \item \opt{only-regex} 将排除那些不匹配正则表达式的元素，这个正则表达式保存在 \cs{physicxexcept} 中；
    \item 它的值也可以是一个宏的名称，它使用这个宏来处理。这个宏应有三个参数。
  \end{itemize}

  注意 \opt{element-code*} 会重定义 \cs{physicx@matrixelement}。
\end{function}

\begin{function}{element-except,element-except+}
这个键设置（或附加）\cs{physicxexcept} 的值，这个宏的值用于 \opt{element-code*} 的相关设置中。
\end{function}
...
\begin{function}{expand-element}
    ...
    \end{syntax}
    是否对生成的矩阵值进行展开，一般情况是不需要的，当使用 \opt{saveto} 保存到宏中，则可能需要启用该选项。
\end{function}
...
\changes{v0.3.3}{2022/07/30}{修改 \opt{sepdim} 的行为，在其为 0pt 时，不显示使用 \cmd{\\}\texttt{[0.0pt]}。}
